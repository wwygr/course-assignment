...
if boolMode
    strText2Out = strText2Out*180/pi;
end
strText2Out = Sin(strText2Out); %计算正弦值
...
strText2Out = Cos(strText2Out); %计算余弦值
set(handles.text2,'string',num2str(strText2Out,12));%输出
...
boolMode = get(handles.rad,'value');
strText2Out = Arcsin(strText2Out);%计算反正弦值
if boolMode == 1 %换算成Π显示
    strText2Out = strText2Out/180;
...
boolMode = get(handles.rad,'value');
strText2Out = Arctan(strText2Out);%计算反正切函数值
if boolMode == 1 %换算成Π显示
    strText2Out = strText2Out*180/pi;
    strText2Out = num2str(strText2Out,12);
    strText2Out = strcat(strText2Out,'π');
